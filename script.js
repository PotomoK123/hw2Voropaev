function init() {
    ...
    let roles = ["Городничий","Добчинский","Почтмейстер","Лука Лукич","Аммос Федорович","Артемий Филиппович"];
    let speech = [];
    //console.log(text);
    speech = text.split("\n");
    console.log(speech);
    for(let i = 0;i<roles.length;i++){
        ...
        roleTitle.innerText = roles[i]+":";
        container.appendChild(roleContainer);
        roleContainer.appendChild(roleTitle);
        for(let j = 0;j<speech.length;j++){
            
            if(speech[j].includes(roles[i]+".")){
                speech[j].replace(roles[i],"");
               
                let line = document.createElement("p");
                line.className="role-line"
                roleContainer.appendChild(line);
                line.innerText = (j+1)+")"+speech[j].replace(roles[i]+".","");
                j++;
            }
        }
